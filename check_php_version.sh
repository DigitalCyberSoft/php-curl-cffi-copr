...
CI_VERSION_FILE="$SCRIPT_DIR/ci_version"
...
check_ci_version() {
    ...
    if [ -f "$CI_VERSION_FILE" ]; then
        saved_ver=$(cat "$CI_VERSION_FILE" | tr -d '[:space:]')
    fi

    echo "  curl-impersonate version: ${new_ver} [saved: ${saved_ver:-none}]"
...
commit_and_push() {
    ...
    git add php_version.json release ext_version ci_version
...
# Check curl-impersonate version
echo "Checking curl-impersonate version..."
if check_ci_version; then
    CI_VERSION_CHANGED=1
    VERSIONS_CHANGED=1
    CHANGE_SUMMARY="${CHANGE_SUMMARY}  curl-impersonate version changed\n"
fi
echo
